...
export default function CashPaymentVoucherList() {
  ...
  const handleView = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await api.get('/cash-payment-vouchers', { params: { fromDate, toDate } });
      setVouchers(res.data);
      setHasSearched(true);
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to load cash payment vouchers.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (voucher) => {
    ...
    try {
      await api.delete(`/cash-payment-vouchers/${voucher._id}`);
      refreshList();
    } catch (err) {
      console.error(err);
    ...
    <div style={{ maxWidth: 1150, margin: '0 auto', fontFamily: 'Arial, sans-serif' }}>
      ...
      {hasSearched && !loading && (
        vouchers.length > 0 ? (
          ...
          <div style={styles.emptyState}>No cash payment vouchers found for that date range.</div>
        )
      )}
      ...
      {editingId && (
        <div style={styles.overlay} onClick={closeEditModal}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <CashPaymentVoucherForm voucherId={editingId} onSaved={handleSavedEdit} onCancel={closeEditModal} />
          </div>
        </div>
      )}
    </div>
