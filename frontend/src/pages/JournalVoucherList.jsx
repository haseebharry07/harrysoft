...
export default function JournalVoucherList() {
  ...
  const handleView = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await api.get('/journal-vouchers', { params: { fromDate, toDate } });
  ...
  const handleDelete = async (voucher) => {
    ...
    try {
      await api.delete(`/journal-vouchers/${voucher._id}`);
      refreshList();
    } catch (err) {
      console.error(err);
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', fontFamily: 'Arial, sans-serif' }}>
      ...
      {hasSearched && !loading && (
        vouchers.length > 0 ? (
            ...
            <tbody>
              {vouchers.map((v) => (
                <tr key={v._id}>
                  ...
                  <td style={styles.td}>{v.voucherType}</td>
          ...
          <div style={styles.emptyState}>No journal vouchers found for that date range.</div>
        )
      )}
      ...
      {editingId && (
        <div style={styles.overlay} onClick={closeEditModal}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <JournalVoucherForm voucherId={editingId} onSaved={handleSavedEdit} onCancel={closeEditModal} />
          </div>
        </div>
      )}
    </div>
...
const styles = {
  ...
  modal: { background: '#fff', borderRadius: 8, width: '100%', maxWidth: 970, margin: '0 16px 40px' }
};