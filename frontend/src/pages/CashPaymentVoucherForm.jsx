...
// Update this if your Cash Payment Voucher list page is registered at a different path.
const CASH_PAYMENT_VOUCHER_LIST_ROUTE = '/cash-payment-vouchers/list';
 ...
 */
export default function CashPaymentVoucherForm({ voucherId = null, onSaved, onCancel }) {
  ...
  const loadForCreate = async () => {
    const res = await api.get('/cash-payment-vouchers/next-voucher-no');
  ...
  const loadForEdit = async (accountList = accounts) => {
    const res = await api.get(`/cash-payment-vouchers/${voucherId}`);
    const v = res.data;
  ...
  const cashCredit = totals.dr;
  const totalsMatch = totals.dr > 0;
  ...
  const validate = () => {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      if (dr <= 0 || cr > 0) return `Row ${i + 1}: enter a DR amount only.`;
    }
    return '';
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let res;
      if (isEditMode) {
        res = await api.put(`/cash-payment-vouchers/${voucherId}`, payload);
      } else {
        res = await api.post('/cash-payment-vouchers', payload);
      }
  ...
  return (
    <div style={styles.card}>
      <div style={styles.titleRow}>
        <h2 style={{ margin: 0 }}>{isEditMode ? 'Edit Cash Payment Voucher' : 'Cash Payment Voucher'}</h2>
        {!isEditMode && (
          <button
            type="button"
            style={styles.viewListButton}
            onClick={() => navigate(CASH_PAYMENT_VOUCHER_LIST_ROUTE)}
          >
            View All Vouchers
          </button>
        )}
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={styles.headerRow}>
          ...
          <div style={styles.headerField}>
            ...
            <input type="text" readOnly value={cashInHand === null ? 'N/A' : (cashInHand - cashCredit).toLocaleString()} style={{ ...styles.input, background: '#eff6ff', fontWeight: 'bold' }} />
          </div>
        ...
        <div style={{ marginTop: 18 }}>
          <p style={{ margin: '0 0 10px', color: '#1d4ed8', fontSize: 13 }}>Cash in Hand / Petty Cash will be credited automatically. Select the expense or other account and enter its debit amount.</p>
        ...
        <div style={styles.totalsRow}>
          ...
          <span>
            Total CR: <strong>{cashCredit.toFixed(2)}</strong>
          </span>
          <span style={{ color: totalsMatch ? '#15803d' : '#b91c1c', fontWeight: 'bold' }}>
            {totalsMatch ? 'Balanced automatically' : 'Enter a debit amount'}
          </span>
        </div>
